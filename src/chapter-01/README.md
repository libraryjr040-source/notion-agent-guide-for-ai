...
所有实体通过 **compressed URL** 标识——形如 `"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"`、`"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"`、`"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"`。这是 API 调用中引用实体的唯一标准方式；系统在处理时自动解压为真实 URL。永远不要用裸字符串（如 `"abc123"`）替代 compressed URL。
...
| `"page"` | 嵌套在另一个页面下 | `{ type: "page", url: "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40" }` |
| `"dataSource"` | 数据库中的一行 | `{ type: "dataSource", url: "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40" }` |
| `"teamspace"` | 团队空间顶层 | `{ type: "teamspace", url: "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40" }` |
...
- ⚠️ Database 名称不是标识符。所有引用必须用 compressed URL（如 `"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"`），不是显示名称。
...
| identifier 形式 | 含义 |
|-----------------|------|
| `"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"` 或 `"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"` | 对特定页面/数据库的权限 |
| `["agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40", "dataSourceUrl"]` | 对多个资源的权限 |
| `"*"` | workspace 级全局权限 |
| `{ type: "agent", url: "dataSourceUrl" }` | 与另一个 agent 的 interact 权限 |
| `"webSearch"` | 网络搜索权限 |
...
- **Custom Agent** → Custom Agent：发起方的 Notion integration 中必须声明 `{ identifier: { type: "agent", url: "dataSourceUrl" }, actions: ["interact"] }`。
...
| Agent 间协作实战 | Ch4 S4.5, Ch5 S5.1（若已加载） |
...
- 行内标记：`Some fact.[^agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40]`
- 多个来源：`Important fact.[^agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40][^dataSourceUrl]`
- 不要添加脚注定义行
- 只有搜索结果中的 URL 可以 cite，不能 cite 工具调用本身的 URL（如 `agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40`）
...
| 编辑 Agent 自身配置 | Agent 不应自行修改自身配置（这是设计策略而非 API 硬限制） |
...
│   ├── dataSourceTypes.ts   ← Property schema 全部 26 种类型定义
...
| Property | DataSource schema 中的一列，26 种类型 | Ch1 S1.1 |
