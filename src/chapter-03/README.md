...
- **compressed URL**：所有已存在实体的唯一标识，格式如 `"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"`、`"dataSourceUrl"`、`"okrs"`。API 调用中引用已有实体必须用 compressed URL，不可用 name。
...
- ⚠️ **View 的 dataSourceUrl 必须引用存在的 key**：可以是同一调用中的 `CREATE-*` key，也可以是已有的 `"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"`。
...
| 单选标签 | `select` | 需预定义 options |
| 多选标签 | `multi_select` | 需预定义 options |
| 工作流状态 | `status` | 有 to_do / in_progress / complete 三组（另有 current / future 可选） |
| 数值 | `number` | 可配 number_format（货币/百分比等）和 show_as（bar/ring） |
| 日期/时间 | `date` | 可配 date_format、time_format、default_reminder |
...
| 地点 | `place` | map view 的 mapBy 必须引用 place property |
| 按钮 | `button` | — |
| 验证状态 | `verification` | — |

### Status Groups 设计

status property 的 `groups` 对象有五个可选组：

```js
groups: {
  to_do: [{ name: "Backlog" }, { name: "Not started" }],
  in_progress: [{ name: "In progress" }, { name: "In review" }],
  complete: [{ name: "Done" }, { name: "Archived" }]
}
```

每组内可以有多个 option。`current` 和 `future` 是额外可用的组（不常用）。
...
可用颜色：`blue`、`brown`、`default`、`gray`、`green`、`orange`、`pink`、`purple`、`red`、`yellow`。
...
"CREATE-project": {
  type: "relation",
  name: "Project",
  dataSourceUrl: "URL",  // 目标数据源的 compressed URL
  limit: 1  // 可选，限制只关联一条
}
```
...
"CREATE-task-count": {
  type: "rollup",
  name: "Task Count",
  relationPropertyUrl: "integration://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40/notion/86b06b1b-9c54-4a7f-ac10-fefcf3d177a9",  // 本数据源上的 relation property
  targetPropertyUrl: "agent://755c9fa4-4e97-8185-a342-00033edae600/00429878-83b7-4209-8fc4-4f4757072e75",    // 目标数据源上要聚合的 property
  aggregation: "count"
}
```

支持的 aggregation：`average`、`checked`、`count`、`count_values`、`date_range`、`earliest_date`、`empty`、`latest_date`、`max`、`median`、`min`、`not_empty`、`percent_checked`、`percent_empty`、`percent_not_empty`、`percent_unchecked`、`range`、`show_unique`、`sum`、`unchecked`、`unique`。还支持 status group 对象：`{ operator: "count_per_group", groupName: "Done" }`。
...
- 所有 property 类型定义 → `modules/notion/databases/dataSourceTypes.ts`
...
所有 view 类型（chart 和 dashboard 除外）都支持：

- **`displayProperties`**：`Array<string>`，按 property 名列出要显示的列
- **`simpleFilters`**：快速筛选，每个元素包含一个 `filter: PropertyFilter`
- **`advancedFilter`**：`ViewFilter` 对象，支持 `and` / `or` 嵌套组合
- **`sorts`**：`Array<{ property: string, direction: "ascending" | "descending" }>`
- **`groupBy`**：按 property 分组（不同 property 类型需要不同的 groupBy 配置，见下文）

### GroupBy 配置（按 property 类型）

不同 property 类型的 groupBy 有不同的必填字段：

| propertyType | 必填字段 | sort 选项 |
|-------------|----------|----------|
| `"status"` | `groupBy: "group" \| "option"` | manual / ascending / descending |
| `"select"` / `"multi_select"` | 无 groupBy 字段 | manual / ascending / descending |
| `"date"` / `"created_time"` / `"last_edited_time"` | `groupBy: "relative" \| "day" \| "week" \| "month" \| "year"` | ascending / descending |
| `"title"` / `"text"` / `"url"` 等 | `groupBy: "exact" \| "alphabet_prefix"` | manual / ascending / descending |
| `"number"` | `groupBy?: { type: "unique", ... }`, 需 `start`, `end`, `size` | ascending / descending |
| `"person"` | 无 | manual |
| `"relation"` | 无 | manual / ascending / descending |
| `"checkbox"` | 无 | manual |
...
- 所有 View 类型定义 + Filter / GroupBy / Sort / Chart → `modules/notion/databases/viewTypes.ts`
...
- **表名 = DataSource 的 compressed URL**，必须用双引号包裹：`SELECT * FROM "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"`
...
| Property 类型 | SQL 类型 | 值格式 |
|--------------|----------|--------|
| title / text / url / email / phone | TEXT | 普通字符串 |
| number | FLOAT | 数值 |
| checkbox | TEXT | `"__YES__"` / `"__NO__"` |
| select / status | TEXT | option name 字符串 |
| multi_select | TEXT | JSON 数组字符串 `["tag1","tag2"]` |
| person | TEXT | JSON 字符串（单人或数组），值为 user compressed URL |
| relation | TEXT | JSON 字符串（单页或数组），值为 page compressed URL |
| date | 展开为 3 列 | `"date:<Name>:start"` / `"date:<Name>:end"` / `"date:<Name>:is_datetime"` |
| place | 展开为 5 列 | `"place:<Name>:address"` / `"place:<Name>:name"` / ... |
| created_time / last_edited_time | TEXT (NOT NULL) | ISO-8601 |
| created_by / last_edited_by | TEXT | user compressed URL |
| auto_increment_id | INTEGER | 数值 |
...
- ⚠️ **place 列同理**：展开成 5 个 `"place:<Name>:*"` 列。
...
- SQL 列名映射完整规范 → `modules/notion/databases/data-source-sqlite-tables.md`
...
```js
{ command: "delete", path: ["views", "okrs"] }
```
...
await connections.notion.updateDatabase({
  ...
  edits: [
    { command: "set",
      path: ["dataSources", "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40", "schema", "URL", "options"],
      value: [
        { name: "P0", color: "red" },
        { name: "P1", color: "orange" },
        { name: "P2", color: "blue" },
...
await connections.notion.updateDatabase({
  databaseUrl: "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40",
  edits: [
    { command: "delete", path: ["views", "dataSourceUrl"] }
  ]
})
```
...
- ⚠️ **path 中引用已有实体用 compressed URL**：如 `["dataSources", "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40", "schema", "URL"]`，不要用 display name。
...
```js
// 假设已通过 loadDatabase 获得源数据源 URL: "OPTIONAL_NOTICE"
await connections.notion.createDatabase({
  name: "My Tasks",
  ...
  views: {
    "CREATE-filtered-view": {
      ...
      dataSourceUrl: "OPTIONAL_NOTICE",  // 引用外部数据源
      advancedFilter: {
        type: "group", operator: "and",
        filters: [
...
const result = await connections.notion.createTwoWayRelation({
  sourceDataSourceUrl: "okrs",
  targetDataSourceUrl: "teams",
...
- ⚠️ **参数是 DataSource URL，不是 Database URL**：传 `"okrs"`，不是 `"agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"`。
...
```
根据内部文档，Q3 目标已更新。[^search-result-url]
```
...
- ⚠️ **citation 引用 result URL，不引用工具调用 URL**：`[^123]` ❌ → `[^result-url]` ✅。
...
await connections.notion.createPage({
  parent: { type: "page", url: "wiki-page-url" },  // wikiPageUrl 的值
