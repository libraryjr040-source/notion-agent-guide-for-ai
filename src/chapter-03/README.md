...
1. **确定 parent**：`{ type: "page", url: "page-1" }` / `{ type: "user", url: "user-1" }` / `{ type: "teamspace", url: "teamspace-1" }`
...
- ⚠️ **View 的 dataSourceUrl 必须引用存在的 key**：可以是同一调用中的 `CREATE-*` key，也可以是已有的 compressed URL 如 `"data-source-1"`。
...
| 地理位置 | `place`（又称 location） | map view 的 mapBy 必须引用 place property；值展开为 5 列，详见 `data-source-sqlite-tables.md` |
| 按钮 | `button` | — |
| 验证状态 | `verification` | — |
...
```js
const result = await connections.notion.querySql({
  dataSourceUrls: ["data-source-2"],
  query: `
    SELECT url, "Task", "Tags"
    FROM "data-source-2" t
    WHERE EXISTS (
...
```js
const result = await connections.notion.querySql({
  dataSourceUrls: ["data-source-3", "data-source-4"],
  query: `
    SELECT t.url, t."Task Name", p."Project Name"
    FROM "data-source-3" t
    JOIN "data-source-4" p
...
1. **加载数据库**：`connections.notion.loadDatabase({ url: "database-1" })` → 获取当前 configuration
...
```js
{ command: "set", path: ["dataSources", "data-source-1", "schema", "CREATE-tags"],
...
```js
{ command: "delete", path: ["views", "view-1"] }
```
...
await connections.notion.updateDatabase({
  ...
  edits: [
    // 添加 property
    { command: "set",
      path: ["dataSources", "data-source-1", "schema", "CREATE-points"],
...
await connections.notion.updateDatabase({
  ...
  edits: [
    { command: "set",
      path: ["dataSources", "data-source-1", "schema", "property-3", "options"],
      value: [
        { name: "P0", color: "red" },
        { name: "P1", color: "orange" },
        { name: "P2", color: "blue" },
...
- ⚠️ **path 中引用已有实体用 compressed URL**：如 `["dataSources", "data-source-1", "schema", "property-3"]`，不要用 display name。
- ⚠️ **path 中新建实体用 CREATE-\***：如 `["dataSources", "data-source-1", "schema", "CREATE-new-prop"]`。
...
```js
// 假设已通过 loadDatabase 获得源数据源 URL: "data-source-1"
await connections.notion.createDatabase({
  name: "My Tasks",
  parent: { type: "page", url: "page-1" },
  inline: true,
  ...
  views: {
    "CREATE-filtered-view": {
      ...
      dataSourceUrl: "data-source-1",  // 引用外部数据源
      advancedFilter: {
        type: "group", operator: "and",
        filters: [
...
const result = await connections.notion.createTwoWayRelation({
  sourceDataSourceUrl: "data-source-1",
  targetDataSourceUrl: "data-source-2",
...
- ⚠️ **参数是 DataSource URL，不是 Database URL**：传 `"data-source-1"`，不是 `"database-1"`。
...
await connections.notion.createPage({
  parent: { type: "page", url: "page-1" },  // wikiPageUrl 的值
...
await connections.notion.createPage({
  parent: { type: "dataSource", url: "data-source-1" },  // 这会失败！
  properties: { title: "New Wiki Article" }
})
```
