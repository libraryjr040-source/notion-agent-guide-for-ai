...
- **compressed URL**：所有已存在实体的唯一标识，格式如 `"https://www.notion.so/337c9fa44e978052a7fae5508bd47d1c"`（page）、`"collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"`（data source）、`"view://336c9fa4-4e97-80ec-9d27-000c7a690979"`（view）。前缀语义匹配实体类型（`page-`、`data-source-`、`database-`、`view-`、`property-`、`user-`、`agent-`、`teamspace-`）。API 调用中引用已有实体必须用 compressed URL，不可用 name。
...
1. **确定 parent**：`{ type: "page", url: "https://www.notion.so/337c9fa44e978052a7fae5508bd47d1c" }` / `{ type: "user", url: "user://332d872b-594c-816c-a9a9-000276e2496b" }` / `{ type: "teamspace", url: "https://www.notion.so/88a395759257472b89f9d38ae3a1df2f" }`
...
await connections.notion.createDatabase({
  ...
  parent: { type: "page", url: "https://www.notion.so/337c9fa44e978052a7fae5508bd47d1c" },
  dataSources: {
    "CREATE-main": {
      url: "CREATE-main",
      name: "Projects",
...
- ⚠️ **View 的 dataSourceUrl 必须引用存在的 key**：可以是同一调用中的 `CREATE-*` key，也可以是已有的 compressed URL 如 `"collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"`。
...
| 单选标签 | `select` | 需预定义 options（含 name 和可选 color） |
| 多选标签 | `multi_select` | 需预定义 options |
| 工作流状态 | `status` | 有 to_do / in_progress / complete 三组，每组可含多个 option |
...
| 地理位置 | `place`（又称 location） | map view 的 mapBy 必须引用 place property；值展开为 5 列（address / name / latitude / longitude / google_place_id） |
| 按钮 | `button` | — |
| 验证状态 | `verification` | — |
...
"CREATE-project": {
  type: "relation",
  name: "Project",
  dataSourceUrl: "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470",  // 目标数据源的 compressed URL
  limit: 1  // 可选，限制只关联一条
}
```
...
"CREATE-task-count": {
  type: "rollup",
  name: "Task Count",
  relationPropertyUrl: "collectionProperty://336c9fa4-4e97-8023-bcdc-000b1d4b5470/S0xJWg",  // 本数据源上的 relation property
  targetPropertyUrl: "collectionProperty://336c9fa4-4e97-8023-bcdc-000b1d4b5470/dGl0bGU",    // 目标数据源上要聚合的 property
  aggregation: "count"
}
```

aggregation 支持 `count`、`sum`、`average`、`min`、`max` 等多种模式，以及 status group 对象。完整列表见 `dataSourceTypes.ts`。
...
- ⚠️ **place property 的值设置**：不能直接设 `"Location": "..."`，必须使用展开的 `place:<Name>:*` 子列（见 S3.4 踩坑清单和 `data-source-sqlite-tables.md`）。

### 深钻指针

- 所有 property 类型定义（含 select 颜色列表、status groups 详细结构、rollup aggregation 完整列表） → `modules/notion/databases/dataSourceTypes.ts`
...
所有 view 类型（chart 和 dashboard 除外）都支持 `displayProperties`、`simpleFilters`、`advancedFilter`、`sorts`、`groupBy`。不同 property 类型的 groupBy 需要不同配置——完整对照表见 `viewTypes.ts`。
      ...
      widgets: [
        {
          ...
          view: {
            ...
            dataSourceUrl: "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470",
            chartConfig: { /* ... */ }
          }
        },
        {
...
- ⚠️ **map view 需要 mapBy**：必须引用一个 place 类型 property 的 name。DataSource 中必须有 place property。

### 深钻指针

- 所有 View 类型定义 + Filter / GroupBy / Sort / Chart + GroupBy 按 property 类型的配置对照表 → `modules/notion/databases/viewTypes.ts`
...
2. **构造 SQL**：使用 SQLite 方言
3. **调用 `connections.notion.querySql`**

### SQLite 方言关键规则

- **表名 = DataSource 的 compressed URL**，必须用双引号包裹：`SELECT * FROM "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"`
- **列名 = property 的 display name**，有空格/特殊字符时用双引号包裹：`"Due Date"`
- **系统列**：`url`（页面 URL，TEXT）、`createdTime`（ISO-8601，TEXT）
- **参数化查询**：用 `?` 占位 + `params` 数组传值
- **完整的列名映射、值编码规范**见深钻指针 `data-source-sqlite-tables.md`
...
const result = await connections.notion.querySql({
  dataSourceUrls: ["collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"],
  query: `
    SELECT url, "Project Name", "Status", "Owner"
    FROM "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"
    WHERE "Status" = ?
...
```js
const result = await connections.notion.querySql({
  dataSourceUrls: ["collection://85fe022d-f83f-47c2-88c4-3c150f5a9032"],
  query: `
    SELECT url, "Task", "Tags"
    FROM "collection://85fe022d-f83f-47c2-88c4-3c150f5a9032" t
    WHERE EXISTS (
...
```js
const result = await connections.notion.querySql({
  dataSourceUrls: ["collection://340c9fa4-4e97-8020-b323-000bd2e94488", "collection://342c9fa4-4e97-80bf-a37e-000bdfa05e1d"],
  query: `
    SELECT t.url, t."Task Name", p."Project Name"
    FROM "collection://340c9fa4-4e97-8020-b323-000bd2e94488" t
    JOIN "collection://342c9fa4-4e97-80bf-a37e-000bdfa05e1d" p
...
- ⚠️ **表名必须双引号包裹**：`FROM "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"` ✅，`FROM collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470` ❌。
...
- SQL 列名映射完整规范（所有 property 类型 → SQL 类型 + 值编码对照表） → `modules/notion/databases/data-source-sqlite-tables.md`
...
const result = await connections.notion.queryView({
  viewUrl: "view://336c9fa4-4e97-80ec-9d27-000c7a690979"
})
...
1. **加载数据库**：`connections.notion.loadDatabase({ url: "https://www.notion.so/336c9fa44e978016aa46e7760a6d61ca" })` → 获取当前 configuration
...
```js
{ command: "set", path: ["dataSources", "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470", "schema", "CREATE-tags"],
...
```js
{ command: "delete", path: ["views", "view://336c9fa4-4e97-80ec-9d27-000c7a690979"] }
```
...
await connections.notion.updateDatabase({
  databaseUrl: "https://www.notion.so/336c9fa44e978016aa46e7760a6d61ca",
  edits: [
    // 添加 property
    { command: "set",
      path: ["dataSources", "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470", "schema", "CREATE-points"],
...
await connections.notion.updateDatabase({
  ...
  edits: [
    { command: "set",
      path: ["dataSources", "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470", "schema", "collectionProperty://336c9fa4-4e97-8023-bcdc-000b1d4b5470/Pkp5ag", "options"],
      value: [
        { name: "P0", color: "red" },
        { name: "P1", color: "orange" },
        { name: "P2", color: "blue" },
...
- ⚠️ **path 中引用已有实体用 compressed URL**：如 `["dataSources", "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470", "schema", "collectionProperty://336c9fa4-4e97-8023-bcdc-000b1d4b5470/Pkp5ag"]`，不要用 display name。
- ⚠️ **path 中新建实体用 CREATE-\***：如 `["dataSources", "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470", "schema", "CREATE-new-prop"]`。
...
```js
// 假设已通过 loadDatabase 获得源数据源 URL: "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"
await connections.notion.createDatabase({
  name: "My Tasks",
  parent: { type: "page", url: "https://www.notion.so/337c9fa44e978052a7fae5508bd47d1c" },
  inline: true,
  ...
  views: {
    "CREATE-filtered-view": {
      ...
      dataSourceUrl: "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470",  // 引用外部数据源
      advancedFilter: {
        type: "group", operator: "and",
        filters: [
...
const result = await connections.notion.createTwoWayRelation({
  sourceDataSourceUrl: "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470",
  targetDataSourceUrl: "collection://85fe022d-f83f-47c2-88c4-3c150f5a9032",
...
- ⚠️ **参数是 DataSource URL，不是 Database URL**：传 `"collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"`，不是 `"https://www.notion.so/336c9fa44e978016aa46e7760a6d61ca"`。
...
await connections.notion.createPage({
  parent: { type: "page", url: "https://www.notion.so/337c9fa44e978052a7fae5508bd47d1c" },  // wikiPageUrl 的值
...
await connections.notion.createPage({
  parent: { type: "dataSource", url: "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470" },  // 这会失败！
  properties: { title: "New Wiki Article" }
})
```
...
const timeSeries = await connections.notion.getPageAnalyticsTimeSeries({
  pageUrl: "https://www.notion.so/337c9fa44e978052a7fae5508bd47d1c",
  days: 30
})
...
const content = await connections.notion.listContentAnalytics({
  ...
  filters: {
    inTeamspaces: ["https://www.notion.so/88a395759257472b89f9d38ae3a1df2f"]
  }
})
```
