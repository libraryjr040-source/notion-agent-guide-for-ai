...
await connections.notion.createPage({
  ...
  pageTemplate: "okrs",  // 模板页面的 compressed URL
...
// db.configuration.wikiPageUrl === "teams"

await connections.notion.createPage({
  parent: { type: "page", url: "teams" },  // wikiPageUrl，不是 dataSource URL
...
await connections.notion.updatePage({
  ...
  propertyUpdates: {
    ...
    "Is Urgent": true,
    Tags: ["Bug", "P0"],
    Assignee: "dataSourceUrl",
    Reviewers: ["dataSourceUrl", "okrs"],
...
await connections.notion.updatePage({
  url: "agent://755c9fa4-4e97-8185-a342-00033edae600/00429878-83b7-4209-8fc4-4f4757072e75",
  propertyUpdates: {
    "Parent Task": "agent://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e"  // limit 1
  }
})
...
await connections.notion.updatePage({
  url: "agent://755c9fa4-4e97-8185-a342-00033edae600/00429878-83b7-4209-8fc4-4f4757072e75",
  propertyUpdates: {
    "Sub-tasks": ["https://www.notion.so/33ec9fa44e9780138c89f8c2f905d804", "integration://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40/mcpServer/e79d39d7-a57c-40ec-9ded-60470b1fc2a9", "trigger://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40/2821c2ba-e01a-4213-9082-ad754ec83a09"]  // 多选
  }
})
```
...
await connections.notion.updatePage({
  ...
  parent: { type: "page", url: "integration://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e/notion/03ea71d9-94ee-4903-9684-b06d4d30d0f3" }
})

// 移到数据源中
await connections.notion.updatePage({
  url: "https://www.notion.so/766e5b1dbb794308a6133ebc00b307b4",
  parent: { type: "dataSource", url: "okrs" }
})

// 移到 teamspace 顶层
await connections.notion.updatePage({
  url: "https://www.notion.so/766e5b1dbb794308a6133ebc00b307b4",
  parent: { type: "teamspace", url: "dataSourceUrl" }
})
```
...
```ts
await connections.notion.createPage({
  parent: { type: "dataSource", url: "teams" },
  asTemplate: true,
...
await connections.notion.updateDatabase({
  ...
  edits: [{
    command: "set",
    path: ["dataSources", "teams", "default_page_template"],
...
```ts
await connections.notion.updatePage({
  url: "user://332d872b-594c-816c-a9a9-000276e2496b",
...
```ts
await connections.notion.deletePages({
  pageUrls: ["https://www.notion.so/336c9fa44e978016aa46e7760a6d61ca", "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470"]
})
```
...
<video src="okrs">视频标题</video>

<pdf src="teams">PDF 文档</pdf>

<audio src="URL">音频文件</audio>

<file src="example.com">附件</file>
```
...
- ⚠️ **src 中的 URL 格式**：`src` 属性可以接受 compressed URL（如 `src="okrs"`）或完整 URL（如 `src="https://example.com/file.pdf"`）。但**不能**用双花括号包裹完整 URL（如 `src="https://example.com"`），这会导致解析失败。
...
const discussions = await connections.notion.getPageDiscussions({
  pageUrl: "https://www.notion.so/ab2835f54480451ab0de2974ff080795",
...
```ts
await connections.notion.addCommentToDiscussion({
  discussionUrl: "https://www.notion.so/ab2835f54480451ab0de2974ff080795",  // 传页面 URL → 页面级评论
  text: "**注意**：这个页面需要更新数据。"
})
```
...
await connections.notion.addCommentToDiscussion({
  ...
  text: "请查看附件中的截图。",
  attachedFileIds: ["OPTIONAL_NOTICE", "connector-*-1"]
})
```
...
```ts
await connections.notion.createPage({
  parent: { type: "page", url: "collection://85fe022d-f83f-47c2-88c4-3c150f5a9032" },
  icon: "🎤",
