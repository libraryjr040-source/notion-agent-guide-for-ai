...
1. 确定 parent：
   - 顶层私有页面 → `{ type: "user", url: "user://332d872b-594c-816c-a9a9-000276e2496b" }`（传当前用户的 URL）
   - 子页面 → `{ type: "page", url: "https://www.notion.so/7fd3f523da6944cd8119bcfeb2d8df72" }`
   - 数据库内 → `{ type: "dataSource", url: "view://233e69ca-33fe-44b4-96f4-d38fc7f6e1a5" }`
   - teamspace 顶层 → `{ type: "teamspace", url: "thread://755c9fa4-4e97-8185-a342-00033edae600/09d6d118-8d90-4e91-adcf-5a54359b3e2f" }`
...
await connections.notion.createPage({
  parent: { type: "page", url: "https://www.notion.so/7fd3f523da6944cd8119bcfeb2d8df72" },
  icon: "📋",
...
```ts
await connections.notion.createPage({
  parent: { type: "dataSource", url: "view://233e69ca-33fe-44b4-96f4-d38fc7f6e1a5" },
  pageTemplate: "https://www.notion.so/3f80cfa416cc4a6aa7a7495c5011bdb9",  // 模板页面的 compressed URL
...
```ts
// 先加载数据库确认 wiki 状态
const db = await connections.notion.loadDatabase({ url: "agent://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e" })
// db.configuration.isWiki === true
// db.configuration.wikiPageUrl === "https://www.notion.so/7fd3f523da6944cd8119bcfeb2d8df72"

await connections.notion.createPage({
  parent: { type: "page", url: "https://www.notion.so/7fd3f523da6944cd8119bcfeb2d8df72" },  // wikiPageUrl，不是 dataSource URL
...
await connections.notion.updatePage({
  url: "https://www.notion.so/7fd3f523da6944cd8119bcfeb2d8df72",
  contentUpdates: [{
    oldStr: "## 本周进展\n\n- 完成了 API 对接",
    newStr: "## 本周进展\n\n- 完成了 API 对接\n- 新增了搜索功能"
  }]
...
await connections.notion.updatePage({
  ...
  propertyUpdates: {
    ...
    "Is Urgent": true,
    Tags: ["Bug", "P0"],
    Assignee: "user://332d872b-594c-816c-a9a9-000276e2496b",
    Reviewers: ["user://332d872b-594c-816c-a9a9-000276e2496b", "https://www.notion.so/a99c9fa44e978319863a810fdf1fc043"],
    "Related Tasks": ["https://www.notion.so/agent/698ee8ffa78849feb2f4608ee81dfc40?wfv=activity&at=5a45f9a1e1314bfaacc4460f09a29f8e", "https://www.notion.so/3f80cfa416cc4a6aa7a7495c5011bdb9"],
  }
})
```
...
// 日期范围
await connections.notion.updatePage({
  url: "https://www.notion.so/agent/698ee8ffa78849feb2f4608ee81dfc40?wfv=activity&at=5a45f9a1e1314bfaacc4460f09a29f8e",
  propertyUpdates: {
...
await connections.notion.updatePage({
  url: "https://www.notion.so/7fd3f523da6944cd8119bcfeb2d8df72",
  propertyUpdates: {
    "Parent Task": "integration://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40/notion/86b06b1b-9c54-4a7f-ac10-fefcf3d177a9"  // limit 1
  }
})
...
await connections.notion.updatePage({
  url: "https://www.notion.so/agent/698ee8ffa78849feb2f4608ee81dfc40?wfv=activity&at=5a45f9a1e1314bfaacc4460f09a29f8e",
  propertyUpdates: {
    "Sub-tasks": ["agent://755c9fa4-4e97-8185-a342-00033edae600/00429878-83b7-4209-8fc4-4f4757072e75", "https://www.notion.so/33ec9fa44e9780138c89f8c2f905d804", "https://www.notion.so/3f80cfa416cc4a6aa7a7495c5011bdb9"]  // 多选
  }
})
```
...
// 移到另一个页面下
await connections.notion.updatePage({
  url: "integration://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40/mcpServer/e79d39d7-a57c-40ec-9ded-60470b1fc2a9",
  parent: { type: "page", url: "trigger://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40/2821c2ba-e01a-4213-9082-ad754ec83a09" }
})
...
// 移到 teamspace 顶层
await connections.notion.updatePage({
  url: "integration://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e/notion/03ea71d9-94ee-4903-9684-b06d4d30d0f3",
  parent: { type: "teamspace", url: "thread://755c9fa4-4e97-8185-a342-00033edae600/09d6d118-8d90-4e91-adcf-5a54359b3e2f" }
})
```
...
⚠️ Toggle 内的 children 必须缩进（tab），否则不会被包含在 toggle 内。

### 实战案例：Toggle Heading

```markdown
## 可折叠的二级标题 {toggle="true"}
	折叠内容第一行
	折叠内容第二行
```
...
```markdown
<callout icon="💡">
	**提示**：callout 内可以包含多个 block，每个 block 需缩进。
	- 列表也可以在 callout 内
	- 使用 Notion-flavored Markdown 格式化，不用 HTML 标签
</callout>
```

⚠️ Callout 内的格式化用 Markdown（如 `**bold**`），不用 HTML（如 `<strong>`）。
...
⚠️ Table cell 只支持 rich text，不支持其他 block 类型（标题、列表、图片等）。Cell 内的格式化用 Markdown。

### 实战案例：组合使用

以下展示多种高级 block 组合在一个页面中的效果：
...
```ts
// 假设 loadDatabase 返回的 data source 中已有模板 integration://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e/mcpServer/d1895f66-7416-4c01-8aa3-deed0cab8ad3
await connections.notion.updateDatabase({
  databaseUrl: "agent://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e",
  edits: [{
    command: "set",
    path: ["dataSources", "view://233e69ca-33fe-44b4-96f4-d38fc7f6e1a5", "default_page_template"],
    value: "integration://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e/mcpServer/d1895f66-7416-4c01-8aa3-deed0cab8ad3"
  }]
})
```
...
await connections.notion.deletePages({
  pageUrls: ["integration://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e/mcpServer/d1895f66-7416-4c01-8aa3-deed0cab8ad3"]
})
```
...
```ts
const perms = await connections.notion.loadPermissions({ url: "trigger://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e/a00de79b-3d9e-44cc-9156-afa4a5ae4581" })
...
await connections.notion.updatePermission({
  url: "trigger://755c9fa4-4e97-8185-a342-00033edae600/d6640eb9-32d8-4de5-928d-346bbc00b38e/a00de79b-3d9e-44cc-9156-afa4a5ae4581",
  item: {
    type: "user",
    userUrl: "user://332d872b-594c-816c-a9a9-000276e2496b",
...
await connections.notion.archivePages({
  pageUrls: ["user://34cc9fa4-4e97-8194-b96b-0027cef5b078", "https://www.notion.so/337c9fa44e978052a7fae5508bd47d1c", "https://www.notion.so/33ec9fa44e97803cbc8ee59962af9534"]
})
```
...
await connections.notion.unarchivePages({
  pageUrls: ["user://34cc9fa4-4e97-8194-b96b-0027cef5b078", "https://www.notion.so/337c9fa44e978052a7fae5508bd47d1c"]
})
```
...
```ts
const result = await connections.notion.viewFileUrl({ url: "https://www.notion.so/dbd3ca055bd745549e512069f4b001b3" })
...
```markdown
![图片描述](https://www.notion.so/dbd3ca055bd745549e512069f4b001b3)

<video src="https://www.notion.so/be8c9fa44e9783f7af32819ad7e0dbc1">视频标题</video>

<pdf src="https://www.notion.so/3f80cfa416cc4a6aa7a7495c5011bdb9">PDF 文档</pdf>

<audio src="https://www.notion.so/34cc9fa44e978046a039dac0d2fcdb6f">音频文件</audio>

<file src="https://www.notion.so/097b738db90641b099674d06f067c5b1">附件</file>
```
...
- ⚠️ **src 中的 URL 格式**：`src` 属性可以接受 compressed URL（如 `src="https://www.notion.so/be8c9fa44e9783f7af32819ad7e0dbc1"`）或完整 URL（如 `src="https://example.com/file.pdf"`）。但**不能**用双花括号包裹完整 URL（如 `src="https://example.com"`），这会导致解析失败。
...
const discussions = await connections.notion.getPageDiscussions({
  pageUrl: "https://www.notion.so/336c9fa44e978016aa46e7760a6d61ca",
...
await connections.notion.addCommentToDiscussion({
  discussionUrl: "https://www.notion.so/dbd3ca055bd745549e512069f4b001b3",  // 从 getPageDiscussions 返回的讨论 URL
  text: "已修复，请查看最新版本。"
})
```
...
```ts
await connections.notion.addCommentToDiscussion({
  discussionUrl: "https://www.notion.so/336c9fa44e978016aa46e7760a6d61ca",  // 传页面 URL → 页面级评论
  text: "**注意**：这个页面需要更新数据。"
})
```
...
```ts
await connections.notion.addCommentToDiscussion({
  discussionUrl: "https://www.notion.so/dbd3ca055bd745549e512069f4b001b3",
  text: "请查看附件中的截图。",
  attachedFileIds: ["https://www.notion.so/be8c9fa44e9783f7af32819ad7e0dbc1", "https://www.notion.so/097b738db90641b099674d06f067c5b1"]
})
```
...
```ts
await connections.notion.createPage({
  parent: { type: "page", url: "collection://336c9fa4-4e97-8023-bcdc-000b1d4b5470" },
  icon: "🎤",
