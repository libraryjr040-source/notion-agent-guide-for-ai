...
1. 确定 parent：
   - 顶层私有页面 → `{ type: "user", url: "{​{user-1}​}" }`（传当前用户的 URL）
   - 子页面 → `{ type: "page", url: "{​{page-1}​}" }`
   - 数据库内 → `{ type: "dataSource", url: "{​{data-source-1}​}" }`
   - teamspace 顶层 → `{ type: "teamspace", url: "{​{teamspace-1}​}" }`
...
```ts
await connections.notion.createPage({
  parent: { type: "dataSource", url: "{​{data-source-1}​}" },
  pageTemplate: "{​{page-tpl-1}​}",  // 模板页面的 compressed URL
...
// db.configuration.wikiPageUrl === "{​{page-wiki-1}​}"

await connections.notion.createPage({
  parent: { type: "page", url: "{​{page-wiki-1}​}" },  // wikiPageUrl，不是 dataSource URL
...
await connections.notion.updatePage({
  url: "{​{page-4}​}",
  propertyUpdates: {
    "Sub-tasks": ["{​{page-7}​}", "{​{page-8}​}", "{​{page-9}​}"]  // 多选
  }
})
```
...
await connections.notion.updatePage({
  url: "{​{page-4}​}",
  parent: { type: "teamspace", url: "{​{teamspace-1}​}" }
})
```
...
```ts
// 假设 loadDatabase 返回的 data source 中已有模板 {​{page-tpl-1}​}
await connections.notion.updateDatabase({
  databaseUrl: "{​{database-1}​}",
  edits: [{
    command: "set",
    path: ["dataSources", "{​{data-source-1}​}", "default_page_template"],
    value: "{​{page-tpl-1}​}"
  }]
})
```
...
```ts
const perms = await connections.notion.loadPermissions({ url: "{​{page-1}​}" })
...
```ts
const result = await connections.notion.viewFileUrl({ url: "{​{file-1}​}" })
...
<video src="{​{file-2}​}">视频标题</video>

<pdf src="{​{file-3}​}">PDF 文档</pdf>

<audio src="{​{file-4}​}">音频文件</audio>

<file src="{​{file-5}​}">附件</file>
```
...
- ⚠️ **src 中的 URL 格式**：`src` 属性支持两种合法格式：compressed URL（如 `src="{​{file-1}​}"`）和裸完整 URL（如 `src="https://example.com/file.pdf"`）。但**不能**把完整 URL 塞进双花括号里——例如写成 `src="` + `` + `https://example.com` + `` + `"` 这种形式会导致解析失败，因为系统会尝试将花括号内的内容当作 compressed URL 解压，而完整 URL 不是合法的 compressed URL。简言之：要么用 compressed URL `{​{file-1}​}`，要么用裸 URL `https://example.com/file.pdf`，不要混搭。
...
await connections.notion.addCommentToDiscussion({
  discussionUrl: "{​{discussion-1}​}",  // 从 getPageDiscussions 返回的讨论 URL
  text: "已修复，请查看最新版本。"
})
```
...
```ts
await connections.notion.addCommentToDiscussion({
  discussionUrl: "{​{page-1}​}",  // 传页面 URL → 页面级评论
  text: "**注意**：这个页面需要更新数据。"
})
```
