...
- Agent 间通信需要双向 interact 权限：调用方的 Notion integration 需要对目标 Agent 设置 `{ identifier: { type: "agent", url: "agent-2" }, actions: ["interact"] }`
- Sub-thread 的 response 只包含最终文本，不包含中间工具调用的结果

### 标识符系统

- 已有实体使用 compressed URL（如 `"agent-1"`、`"page-1"`、`"database-1"`）
...
| `"page-1"` | `["edit"]` | 特定页面编辑 |
| `"database-1"` | `["view"]` | 特定数据库只读 |
| `["page-1", "page-2"]` | `["edit"]` | 多个页面编辑 |
...
```typescript
{ identifier: { type: "agent", url: "agent-2" }, actions: ["interact"] }
```

要禁止已有的通信权限：

```typescript
{ identifier: { type: "agent", url: "agent-2" }, actions: ["disallow"] }
```
...
const agent = await connections.notion.createAgent({
  ...
  integrations: {
    "CREATE-notion": {
      ...
      name: "Notion",
      permissions: [
        { identifier: "database-1", actions: ["view"] },
        { identifier: "webSearch", actions: ["allow"] },
      ],
    },
  },
...
- ⚠️ **interact 权限是单向的**：Agent A 要调用 Agent B 的 `createAndRunThread`，需要 Agent A 的 Notion integration 中配置 `{ identifier: { type: "agent", url: "agent-2" }, actions: ["interact"] }`。如果 B 也需要回调 A，B 的配置中也需要对 A 的 interact 权限。
...
1. **获取 instructions 页面 URL**：
   ```typescript
   const agent = await connections.notion.loadAgent({ url: "agent-1" })
...
await connections.notion.updateAgent({
  ...
  edits: [
    {
      command: "set",
      path: ["triggers", "trigger-1", "enabled"],
      value: false,
    },
  ],
})
...
await connections.notion.updateAgent({
  agentUrl: "agent-1",
  edits: [
    { command: "delete", path: ["triggers", "trigger-1"] },
  ],
})
```
...
await connections.notion.updateAgent({
  ...
  edits: [
    {
      command: "set",
      path: ["integrations", "integration-1", "permissions"],
      value: [
...
```typescript
const db = await connections.notion.loadDatabase({ url: "database-1" })
// db.autofillCustomAgents → [{ url: "agent-4", name: "Auto Tagger" }]
```
...
permissions: [
  { identifier: "*", actions: ["edit"] },
  { identifier: { type: "agent", url: "agent-2" }, actions: ["interact"] },
  { identifier: { type: "agent", url: "agent-3" }, actions: ["interact"] },
]
```
...
const { threads } = await connections.notion.queryThreads({
  agentUrl: "agent-1",  // 可选，默认当前 Agent
  limit: 10,
...
| interact 权限 | Agent 间通信的授权，identifier 为 { type: "agent", url: "agent-2" }，actions 为 ["interact"] | S4.2 |
