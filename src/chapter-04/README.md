...
- Agent 间通信需要双向 interact 权限：调用方的 Notion integration 需要对目标 Agent 设置 `{ identifier: { type: "agent", url: "agent-B" }, actions: ["interact"] }`
...
```typescript
{ identifier: { type: "agent", url: "agent-B" }, actions: ["disallow"] }
```
...
await connections.notion.updateAgent({
  ...
  edits: [
    {
      command: "set",
      path: ["triggers", "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40", "enabled"],
      value: false,
    },
  ],
})
...
await connections.notion.updateAgent({
  agentUrl: "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40",
  edits: [
    { command: "delete", path: ["triggers", "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40"] },
  ],
})
```
...
| interact 权限 | Agent 间通信的授权，identifier 为 { type: "agent", url: "agent-url" }，actions 为 ["interact"] | S4.2 |
