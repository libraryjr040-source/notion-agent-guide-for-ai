...
```typescript
{ identifier: { type: "agent", url: "456" }, actions: ["interact"] }
```

要禁止已有的通信权限：

```typescript
{ identifier: { type: "agent", url: "456" }, actions: ["disallow"] }
```
...
- ⚠️ **interact 权限是单向的**：Agent A 要调用 Agent B 的 `createAndRunThread`，需要 Agent A 的 Notion integration 中配置 `{ identifier: { type: "agent", url: "agent-B" }, actions: ["interact"] }`。如果 B 也需要回调 A，B 的配置中也需要对 A 的 interact 权限。
...
// 监听数据库上 Agent 配置的变更（用于 autofill agent 重跑等场景）
{
  "CREATE-trigger-agent-updated": {
    ...
    state: {
      ...
      collectionId: "optional-collection-uuid",  // 可选，指定目标数据库的 collection ID
    },
    integrationUrl: "CREATE-notion-1",
  },
}
  ...
  edits: [
    {
      ...
      value: {
        ...
        permissions: [
          ...
          { identifier: "general", actions: ["write"] },
        ],
      },
    },
  ],
...
```typescript
const db = await connections.notion.loadDatabase({ url: "456" })
