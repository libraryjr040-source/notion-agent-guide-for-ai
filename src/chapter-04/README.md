...
- Agent 间通信需要双向 interact 权限：调用方的 Notion integration 需要对目标 Agent 设置 `{ identifier: { type: "agent", url }, actions: ["interact"] }`
- Sub-thread 的 response 只包含最终文本，不包含中间工具调用的结果

### 标识符系统

- 已有实体使用 compressed URL（如 `agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40`、`dataSourceUrl`、`okrs`）
...
**Agent Icon Shapes**：`triangle`、`square`、`bulb`、`chat`、`check`、`hat`、`globe`、`plug`、`sign`、`book`、`apple`、`file`、`ghost`、`unbrella`、`mailbox`、`puzzle`、`alarm`、`rock`

**Agent Icon Colors**：`brown`、`orange`、`yellow`、`green`、`blue`、`purple`、`pink`、`red`、`gray`
...
- Icon shapes/colors 枚举：`modules/notion/agents/index.ts` → `agentIconShapes`、`agentIconColors`
...
| `"toolu_01KxocexkJVcZ4uT7N8bzK7Y"` | `["edit"]` | 特定页面编辑 |
| `"456"` | `["view"]` | 特定数据库只读 |
...
```typescript
{ identifier: { type: "agent", url: "toolu_01KxocexkJVcZ4uT7N8bzK7Y" }, actions: ["interact"] }
```

要禁止已有的通信权限：

```typescript
{ identifier: { type: "agent", url: "toolu_01KxocexkJVcZ4uT7N8bzK7Y" }, actions: ["disallow"] }
```
...
- ⚠️ **interact 权限是单向的**：Agent A 要调用 Agent B 的 `createAndRunThread`，需要 Agent A 的 Notion integration 中配置 `{ identifier: { type: "agent", url: B_url }, actions: ["interact"] }`。如果 B 也需要回调 A，B 的配置中也需要对 A 的 interact 权限。
...
- content 项类型：`text`、`image`、`audio`、`resource`、`resource_link`
...
// 监听特定属性的变更
{
  "CREATE-trigger-page-updated": {
    ...
    state: {
      ...
      debounceTimeoutSeconds: 30,  // 30 秒内多次编辑合并为一次触发
    },
    integrationUrl: "CREATE-notion-1",
  },
}
...
**page.updated 特有参数**：
- `properties`：属性名或属性 URL 数组，仅监听这些属性
- `shouldIgnorePageContentUpdates`：设为 true 则忽略内容变更，只监听属性变更
- `debounceTimeoutSeconds`：防抖秒数，合并短时间内的多次编辑
- `propertyFilters`：更细粒度的属性值过滤

**page.created / page.deleted 特有参数**：
- `viewId`：可选，限定为某个 view 范围内的页面事件
...
1. **确保双方权限配置正确**：
   - 调用方 Agent A 的 Notion integration 中需要：
     ```typescript
     { identifier: { type: "agent", url: "agent-B" }, actions: ["interact"] }
     ```
...
- 需要调整 Agent 的权限范围
- 需要添加/删除 integrations 或 triggers
- 需要修改 Agent 的基本信息

### 推荐路径

`updateAgent` 使用 edits 数组（与 `updateDatabase` 模式相同），每个 edit 操作在 `AgentConfiguration` 上执行。

**三种 edit 命令**：

| 命令 | 用途 | 必需字段 |
|------|------|----------|
| `set` | 设置值（新建或覆盖） | `path` + `value` |
| `delete` | 删除指定路径 | `path` |
| `replaceString` | 字符串替换 | `path` + `replaceStr` + `newStr` + `numOccurrences` |

**path 是相对于 AgentConfiguration 的**——不要在 path 中包含 `"configuration"`、`"agentUrl"` 或 `"instructionsPageUrl"`。
...
await connections.notion.updateAgent({
  ...
  edits: [
    { command: "set", path: ["name"], value: "新名称" },
    { command: "set", path: ["description"], value: "新描述" },
  ],
})
```
  ...
  edits: [
    {
      ...
      value: {
        ...
        permissions: [
          { identifier: "public:*", actions: ["read", "replyInThread"] },
        ],
      },
    },
  ],
...
await connections.notion.updateAgent({
  agentUrl: "agent://755c9fa4-4e97-8185-a342-00033edae600/698ee8ff-a788-49fe-b2f4-608ee81dfc40",
  edits: [
    { command: "delete", path: ["triggers", "okrs"] },
  ],
})
```
...
await connections.notion.updateAgent({
  ...
  edits: [
    {
      command: "set",
      path: ["integrations", "dataSourceUrl", "permissions"],
      value: [
        { identifier: "*", actions: ["view"] },  // 从 edit 降级为 view
        { identifier: "webSearch", actions: ["allow"] },
      ],
    },
  ],
...
- ⚠️ **edits path 不含顶层 key**：path 从 AgentConfiguration 的属性开始。写 `["name"]` 不写 `["configuration", "name"]`。
- ⚠️ **不要用 updateAgent 改 Instructions 内容**：Instructions 是独立的 Notion 页面，用 `updatePage({ url: agent.instructionsPageUrl })` 修改。
- ⚠️ **set 整个 permissions 数组会覆盖**：如果只想加一条权限，需要先 loadAgent 获取当前 permissions，然后 set 完整的新数组。没有 "append" 操作。
- ⚠️ **CREATE-\* 在 updateAgent 中同样有效**：可以在 edits 中同时创建新 integration 和新 trigger，并用 CREATE-* 互相引用。

### 深钻指针

- updateAgent 类型与示例：`modules/notion/agents/index.ts` → `UpdateAgent`
...
// db.autofillCustomAgents → [{ url: "agent-autofill", name: "Auto Tagger" }]
```
...
permissions: [
  { identifier: "*", actions: ["edit"] },
  { identifier: { type: "agent", url: "agent-writer" }, actions: ["interact"] },
  { identifier: { type: "agent", url: "agent-reviewer" }, actions: ["interact"] },
]
```
...
const { analysis } = await connections.notion.investigateThread({
  threadUrl: "toolu_01KxocexkJVcZ4uT7N8bzK7Y",
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `headerContent` | string | 通知标题 |
| `bodyContent` | string | 通知正文，**不超过 100 字符** |
| `userUrl` | string? | 目标用户 URL，省略时发给 workflow owner |
| `sendToWorkflowOwner` | boolean | 是否发送给 workflow owner |
...
| interact 权限 | Agent 间通信的授权，identifier 为 { type: "agent", url }，actions 为 ["interact"] | S4.2 |
